...
const CreateCTokenWithSolanaPayForm = () => {
  const { isPending } = useCreateMint();
  ...
  const { newChargeLink, setChargeId, chargeLinkData, setMetadata } =
    useHelio();
  ...
  async function onSubmit(values: FormSchemaType) {
    ...
    const result = await newChargeLink();
    console.log("result", result);
    if (!result) {
      toast.error("Failed to create charge link");
      return;
    }
    const { chargeId, url } = result;
...
export default CreateCTokenWithSolanaPayForm;
